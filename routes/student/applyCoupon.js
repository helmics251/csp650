...
router.post("/", async (req, res) => {
    try {
        ...
        user.Parcel.forEach((parcel, index) => {
            parcel.price = parseFloat(discountedPriceArray[index].toFixed(2));
        });
        ...
        for (let i = 0; i < parcelIDArray.length; i++) {
            await db.collection("staffs").updateOne({ "locker.parcel.parcelID": parcelIDArray[i] }, { $set: { "locker.$.parcel.price": parseFloat(discountedPriceArray[i].toFixed(2)) } });
        }
